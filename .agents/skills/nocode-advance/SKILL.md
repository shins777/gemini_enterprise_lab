...
name: nocode-advance
description: >-
  Gemini Enterprise Workflow Builder의 빈 캔버스(Empty Canvas)에서 부동산 질문 메일(Gmail) 송신 트리거(When an email is sent), JSON Schema 기반 구조화 출력(질문분석_구조화_에이전트), 한국 부동산 20개년 지표 MCP 서버(MCP Realestate) 연동, 구글 검색 기반 최신 부동산 분석 에이전트(Realest_research) 호출, 종합 보고서 작성(종합_부동산보고서_작성기), 조건 분기(Condition) 및 Human-in-the-Loop 승인(Request info), 분기별 Gemini Agent(Gmail 답변 발송 및 Google Drive 보고서 저장)까지 엔터프라이즈 AI 워크플로 에이전트를 수동(Step-by-Step Manual)으로 조립·검증하는 심화 핸즈온 랩 가이드(`ge_lab/nocode_advance/nocode_advance.md`) 작성·복원 및 실행 스킬입니다.
version: 2.0.0
---

# 🛠️ Skill Execution & Document Reconstruction Guide (`nocode_advance.md` 생성 명세)

이 스킬(`nocode-advance`)은 Gemini Enterprise의 **심화 워크플로 에이전트(Workflow Agent) 개발 및 핸즈온 랩 가이드 문서(`ge_lab/nocode_advance/nocode_advance.md`)**를 작성, 유지보수, 또는 처음부터 완벽하게 역생성(Reconstruct)하기 위한 마스터 블루프린트입니다.

## 1. 문서 생성 및 복원 규칙 (Reconstruction Rules)
사용자가 이 스킬을 기반으로 `nocode_advance.md` 파일 생성을 요청할 경우 다음 규칙을 100% 준수하여 출력해야 합니다:
- **대상 파일 경로**: `ge_lab/nocode_advance/nocode_advance.md`
- **이미지 리소스 경로**: `ge_lab/nocode_advance/resources/img/` (문서 내 상대 경로: `resources/img/파일명.png`)
- **이미지 HTML 태그 표준 스타일**:
  ```html
  <img style="border: 1px solid #e0e0e0; border-radius: 6px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);" src="resources/img/파일명.png" width="600">
  ```
  - 기본 `width="600"`을 사용하며, JSON Schema 전체 표를 보여주는 `lab2-7.png`만 `width="800"`을 유지합니다.
- **핵심 아키텍처 및 명칭 규칙 (필수 준수)**:
  - **시작 트리거 (`Trigger`)**: 이메일 수신(`When an email is received`)이 아닌 **이메일 송신(`When an email is sent`)** (`when_an_email_is_sent`) 사용. (사용자 주도형 트리거링 편의성 및 본인에게 보낸 메일 Self-sent email 미작동 방지 사유 명시)
  - **Step 1 (`질문분석_구조화_에이전트` / 내부 ID `gemini_agent`)**: Google Drive 부동산 세제/공급 문서 2개 Knowledge 바인딩 (Drive `Search for data`는 Disable, `Add or update data`는 Enable / Gmail은 둘 다 Enable), Structured Output 6개 필드(`summary`, `target_region`, `start_year`, `end_year`, `historical_topic`, `priority`) 추출.
  - **Step 2 (`MCP 서버 호출` / 연결 앱 `MCP Realestate`)**: `MCP Realestate` 표기 통일 (`mcp_realestate` 사용 금지). 옵션 1(Gemini Agent 매개 방식)과 옵션 2(직접 MCP 스텝 배치 방식) 모두 수록.
  - **Step 3 (`Realest_research` / `Existing agents`)**: 구글 검색(Google Search)을 통해 최신 부동산 시장 동향·정책·매매/전세 이슈를 검색·분석하는 `Realest_research` 에이전트 호출 (`온고지신` 사용 금지).
  - **Step 4 (`종합_부동산보고서_작성기` / 내부 ID `gemini_agent_2`)**: Step 1 + Step 2(`MCP Realestate`) + Step 3(`Realest_research`) 결과를 통합하여 4개 섹션 마크다운 보고서 작성 (`GeminiAgent_2` 표기 대신 `종합_부동산보고서_작성기` 사용).
  - **Step 5 & 6 (`Condition` & `Request info`)**: `${질문분석_구조화_에이전트:priority}` `equals` `High` 분기 및 `Priority is High` 경로 하단 `Request info` 승인(`승인 (Approve)` / `반려 (Reject)`).
  - **Step 7 (`저장 및 이메일발송` — `High` 경로)**: 별도 Connectors 노드가 아닌 **`Gemini Agent` 노드**를 배치하고 `Connected apps`에서 **Gmail(`On`) + Drive(`On`)**를 모두 활성화하여 질문자에게 이메일 발송 및 Google Drive에 보고서 파일(`.md`) 저장.
  - **Step 8 (`일반 이메일회신` — `Else` 경로)**: **`Gemini Agent` 노드**를 배치하고 `Connected apps`에서 **Gmail(`On`)만 활성화(Drive `Off`)**하여 Google Drive 파일 저장 없이 이메일만 회신.

## 2. 이미지 리소스 매핑 매니페스트 (Total 32 Images)
| 섹션 / 실습 모듈 | 이미지 파일명 (`resources/img/`) | `width` 속성 | 삽입 위치 및 화면 설명 |
| :--- | :--- | :---: | :--- |
| **2. 아키텍처** | `prep2-1.png` | `600` | 섹션 2 Mermaid 아키텍처 다이어그램 직후 ("실제 구현 화면은 아래와 유사합니다.") |
| **Lab 1** | `lab1-0.png` | `600` | Lab 1 ➔ `1)` 3번 항목("빌더로 직접 만들기(Build Manually)") 직후 |
| **Lab 1** | `lab1-1.png` | `600` | Lab 1 ➔ `1)` 4번 항목(워크플로 이름/설명 입력) 직후 |
| **Lab 1** | `lab1-2.png` | `600` | Lab 1 ➔ `2)` `When an email is sent` 중요 설명 박스 직후 (연속 3장 중 1) |
| **Lab 1** | `lab1-3.png` | `600` | Lab 1 ➔ `2)` `lab1-2.png` 직후 (연속 3장 중 2) |
| **Lab 1** | `lab1-4.png` | `600` | Lab 1 ➔ `2)` `lab1-3.png` 직후 (연속 3장 중 3) |
| **Lab 2** | `lab2-1.png` | `600` | Lab 2 ➔ `2) Instructions` 프롬프트 코드블록 및 참고 알림 박스 직후 (연속 2장 중 1) |
| **Lab 2** | `lab2-2.png` | `600` | Lab 2 ➔ `2) Instructions` `lab2-1.png` 직후 (연속 2장 중 2) |
| **Lab 2** | `lab2-3.png` | `600` | Lab 2 ➔ `3) Knowledge` 2번 항목(부동산 파일 2개 선택) 직후 (연속 2장 중 1) |
| **Lab 2** | `lab2-4.png` | `600` | Lab 2 ➔ `3) Knowledge` `lab2-3.png` 직후 (연속 2장 중 2) |
| **Lab 2** | `lab2-5.png` | `600` | Lab 2 ➔ `3) Knowledge` 3번 항목(Drive `Search for data` Disable 설정) 직후 |
| **Lab 2** | `lab2-6.png` | `600` | Lab 2 ➔ `3) Knowledge` 4번 항목(Gmail 권한 Enable 설정) 직후 |
| **Lab 2** | `lab2-7.png` | `800` | Lab 2 ➔ `4) Output format` 2번 항목 직후 (6개 필드 표 바로 위) |
| **Lab 2** | `lab2-8.png` | `600` | Lab 2 ➔ `4) Output format` 6개 필드 표 직후 |
| **Lab 3** | `lab3-1.png` | `600` | Lab 3 ➔ `옵션 1` 2번 항목(`MCP Realestate` 선택) 직후 |
| **Lab 3** | `lab3-2.png` | `600` | Lab 3 ➔ `옵션 1` 3번 항목(`Instructions` 코드블록) 직후 |
| **Lab 4** | `lab4-1.png` | `600` | Lab 4 ➔ `1) [Step 3]` 3번 항목(`Realest_research` 추가) 직후 |
| **Lab 4** | `lab4-2.png` | `600` | Lab 4 ➔ `1) [Step 3]` 4번 항목(`Realest_research` 요청 프롬프트) 직후 |
| **Lab 4** | `lab4-3.png` | `600` | Lab 4 ➔ `2) [Step 4]` `종합_부동산보고서_작성기` 프롬프트 코드블록 직후 |
| **Lab 5** | `lab5-1.png` | `600` | Lab 5 ➔ `1) [Step 5]` 4번 항목(`Condition` 설정) 직후 |
| **Lab 5** | `lab5-2.png` | `600` | Lab 5 ➔ `2) [Step 6]` 2번 항목(`Request info` 추가) 직후 |
| **Lab 5** | `lab5-3.png` | `600` | Lab 5 ➔ `2) [Step 6]` 3번 항목(`Message` 검토 메시지 코드블록) 직후 |
| **Lab 5** | `lab5-4.png` | `600` | Lab 5 ➔ `2) [Step 6]` `Questions` 설정 및 구분선(`---`) 직후 |
| **Lab 6** | `lab6-1.png` | `600` | Lab 6 ➔ `2) [Step 7]` `저장 및 이메일발송` 프롬프트 코드블록 및 구분선 직후 (연속 2장 중 1) |
| **Lab 6** | `lab6-2.png` | `600` | Lab 6 ➔ `2) [Step 7]` `lab6-1.png` 직후 (연속 2장 중 2) |
| **Lab 6** | `lab6-3.png` | `600` | Lab 6 ➔ `3) [Step 8]` `일반 이메일회신` 프롬프트 코드블록 직후 |
| **Lab 6** | `lab6-4.png` | `600` | Lab 6 ➔ `4) 버전 스냅샷 저장 및 프로덕션 활성화` 안내 박스 직후 |
| **Lab 7** | `lab7-1.png` | `600` | Lab 7 도입부 `gmail.com` 로그인 안내 직후 |
| **Lab 7** | `lab7-2.png` | `600` | Lab 7 ➔ `1) Test 시뮬레이션 실행` 2번 항목(보낸 이메일 검색) 직후 |
| **Lab 7** | `lab7-3.png` | `600` | Lab 7 ➔ `1) Test 시뮬레이션 실행` 3번 항목(`Start a test run` 클릭) 직후 |
| **Lab 7** | `lab7-4.png` | `600` | Lab 7 ➔ `4. 단계별 실행 그래프 검증 체크리스트` [Step 2] 항목 직후 |
| **Lab 7** | `lab7-5.png` | `600` | Lab 7 ➔ `4. 단계별 실행 그래프 검증 체크리스트` [Step 5 & 6] 항목 직후 |
| **Lab 7** | `lab7-6.png` | `600` | Lab 7 ➔ `4. 단계별 실행 그래프 검증 체크리스트` [Step 7] 항목 직후 |
| **Lab 7** | `lab7-7.png` | `600` | Lab 7 마지막 문구("아래 그림은 모든 스텝이 최종적으로 완료된 실행 결과 화면입니다.") 직후 |

---

# 📄 `nocode_advance.md` 마스터 원본 콘텐츠 (Master Template)

아래 구분선 이후의 마크다운은 `ge_lab/nocode_advance/nocode_advance.md`를 생성하거나 업데이트할 때 그대로 출력해야 하는 표준 마스터 본문입니다.
