...
name: ge-general
description: >-
  Gemini Enterprise의 10가지 핵심 기능(Direct Q&A, Web Grounding, Enterprise Connectors, 등록된 스킬 사용, 전문 서브 에이전트 위임, MCP 서버 연동, 커스텀 스킬 제작, 프로젝트 워크스페이스, 스마트 수신함, 인터랙티브 캔버스)을 단계별로 검증하는 핸즈온 랩 실습 가이드 스킬입니다.
version: 1.1.0
---

# 🚀 Gemini Enterprise 일반 기능 핸즈온 랩 가이드

본 문서는 **Gemini Enterprise**의 핵심 기능 전반을 실제 엔터프라이즈 환경에서 검증하고 체험할 수 있도록 구성된 종합 실습 가이드입니다.  
Gemini Enterprise의 10가지 핵심 모듈을 바탕으로 환경 설정, UI 인터페이스 구조, 단계별 시나리오, 실습 프롬프트 및 검증 포인트를 제공합니다.
...
- [ ] 모델 응답 맨 하단에 Source 버튼을 클릭했을 때 관련된 참조 웹사이트가 우측에 열리는가?
...
#### 1) 실행 환경 설정
- **Settings > Connectors**에서 Google Drive 및 Gmail, Calendar가 연결되어 있는지 확인합니다. 연결되지 않은 경우 Authorize를 눌러 연결합니다.
- 프롬프트를 입력하기 전 도구 바에서 해당하는 Connector들을 활성화/선택합니다.

#### 2) 실습 프롬프트
> 💬 **프롬프트 1 (사내 문서 작성 및 구글 드라이브 저장):**
> 문서 검색 실습을 위해 아래 두 개의 문서를 작성하여 Google Drive에 저장합니다:
> ```text
...
> ```
> *확인:* 구글 드라이브(https://drive.google.com/)에서 두 개의 문서가 정상 생성되었는지 확인합니다.
...
> 💬 **프롬프트 3 (일정 및 메일 교차 확인 / 전달):**
> ```text
> 위에서 정리한 내용을 요약해서 내 메일로 전달해줘.
> ```
> *확인:* 구글 메일(https://mail.google.com/)에 접속하여 요약 메일이 전송되었는지 확인합니다.
...
#### 1) 실행 환경 설정
- 좌측 메뉴에서 **Skills** 클릭 ➔ **Browse skills** 클릭
- 스킬 목록 중 `/answer-format`을 찾아 **Install** 클릭 후 활성화

#### 2) 실습 프롬프트
- 프롬프트 입력창에서 `@`를 입력하여 방금 등록한 스킬(`/answer-format`)을 선택합니다.
- 선택 후 아래 프롬프트를 입력합니다:
> 💬 **프롬프트 1 (스킬을 활용한 답변 생성):**
> ```text
> 최근 다양한 산업분야에서 AI 에 대한 활용이 늘어나고 있습니다. 제조 산업에서의 특이한 적용분야에 대해서 요약해주세요.
> ```
> *참고:* 특정 스킬을 `@`로 명시적 선택하는 방법 외에도, 프롬프트 내용에 따라 시스템이 자동으로 스킬을 감지하여 호출할 수도 있습니다.
...
특정 업무 수행 시 사전에 공유된 Agent를 통해 업무를 위임하고 처리하는 방법을 실습합니다. 본 실습에서는 사전에 구성된 **"AI 시장 뉴스 및 리스크 브리핑 자동화" Agent**를 호출하여 처리합니다.

#### 1) 실행 환경 설정
- 좌측 네비게이션에서 **Agent** 메뉴를 클릭하여 사용 가능한 에이전트 목록 확인
- 프롬프트 입력창에서 `@`를 입력하여 해당 에이전트를 호출/선택
...
표준화된 **Model Context Protocol (MCP)** 서버를 통해 외부 데이터베이스, 사내 백엔드 시스템, 데이터셋을 호출하고 파라미터를 준수하는지 검증합니다. 본 실습에서는 사전 배포된 한국 부동산 지표 분석 MCP 서버를 호출합니다.
...
- [ ] 처리 과정에서 MCP 호출 과정 및 반환 데이터가 명확하게 반영되는가?

---

### Lab 7. 커스텀 스킬 제작 및 배포 (Custom Skill Authoring)

사내 업무 표준화를 위해 새로운 스킬을 설계하고, **표준 YAML Frontmatter 및 단계별 행동 지침(`SKILL.md`)**을 Gemini 대화를 통해 직접 작성·등록합니다.

#### 1) 실행 환경 설정
- 좌측 메뉴에서 **Skill**을 클릭하여 Gemini 기반 스킬 생성 마법사로 진입합니다.
...
> ```
- 생성된 스킬의 설정을 확인하고 저장합니다.
- 등록된 스킬을 참조하여 프롬프트를 입력하고 의도된 리포트가 산출되는지 확인합니다.
...
#### 1) 실행 환경 설정
- 좌측 사이드바에서 **[Projects] > [+ New Project]**를 클릭하여 새 프로젝트 생성 (예: "AI 기술동향")
- 참조 자료(PDF, 문서 등)를 프로젝트 파일로 업로드
- 함께 작업할 사용자를 추가 (개인 계정 등록 가능, 그룹 계정은 지원되지 않음)
...
- [ ] 여러 턴의 대화나 새 세션이 시작되어도 프로젝트 레벨의 지식정보가 그대로 유지되는가?
...
- 작성된 문서를 바탕으로 구글 슬라이드 연동 및 PPTX 파일 내보내기(Export)가 가능한 상태인지 확인합니다.
...
> 💬 **프롬프트 3 (인라인 편집 및 PPTX 다운로드/내보내기):**
> ```text
> 생성된 슬라이드에서 데이터 수집 섹션을 Cloud Pub/Sub과 BigQuery 스트리밍 인제스천 중심으로 보강한 뒤, 전체 슬라이드를 PPTX 파일로 다운로드할 수 있도록 내보내기(Export) 링크를 준비해줘.
> ```
...
- [ ] 생성된 슬라이드를 로컬 `.pptx` 파일로 다운로드하거나 구글 드라이브(Google Slides)로 직접 내보내기(Export)할 수 있는가?
